...
threads compiling Mesh as you have cores in your CPU, if not one or two more.
...
.. note::
    ...
    Prior to Mesh 4.0, the Linux/\*BSD target was called ``x11`` instead of
    ``linuxbsd``. If you are looking to compile Mesh 3.x, make sure to use the
    `stable branch of this documentation <https://docs.meshengine.org/en/stable/development/compiling/compiling_for_x11.html>`__.
...
.. note:: If you are compiling Mesh for production use, then you can
          make the final executable smaller and faster by adding the
          SCons option ``target=release_debug``.

          If you are compiling Mesh with GCC, you can make the binary
...
::
    ...
    $HOME/.local/share/mesh/templates/[gd-version]/

and named like this (even for \*BSD which is seen as "Linux/X11" by Mesh):
...
files in the ``bin/`` directory of your Mesh source folder, so the next
time you build, you automatically have the custom templates referenced.

Using Clang and LLD for faster development
------------------------------------------

You can also use Clang and LLD to build Mesh. This has two upsides compared to
the default GCC + GNU ld setup:

- LLD links Mesh significantly faster compared to GNU ld or gold. This leads to
