...
def _build_gdnative_api_struct_header(api):
    out = [
        ...
        "#ifndef MESH_GDNATIVE_API_STRUCT_H",
        "#define MESH_GDNATIVE_API_STRUCT_H",
        "",
        "#include <gdnative/gdnative.h>",
        "#include <android/mesh_android.h>",
        "#include <nativescript/mesh_nativescript.h>",
        "#include <pluginscript/mesh_pluginscript.h>",
        "#include <videodecoder/mesh_videodecoder.h>",
        "",
        "#ifdef __cplusplus",
        'extern "C" {',
        "#endif",
    ...
    def generate_extension_struct(name, ext, include_version=True):
        ...
        ret_val += [
            "typedef struct mesh_gdnative_ext_"
            + name
    ...
    def generate_core_extension_struct(core):
        ...
        ret_val += [
            "typedef struct mesh_gdnative_core_"
    ...
    out += [
        "typedef struct mesh_gdnative_core_api_struct {",
        "\tunsigned int type;",
        "\tmesh_gdnative_api_version version;",
        "\tconst mesh_gdnative_api_struct *next;",
        "\tunsigned int num_extensions;",
        "\tconst mesh_gdnative_api_struct **extensions;",
    ]
    ...
    out += [
        "} mesh_gdnative_core_api_struct;",
        "",
        "#ifdef __cplusplus",
        "}",
        "#endif",
        "",
        "#endif // MESH_GDNATIVE_API_STRUCT_H",
        "",
    ]
    return "\n".join(out)
...
def _build_gdnative_api_struct_source(api):
    ...
    def get_extension_struct_definition(name, ext, include_version=True):
        ...
        ret_val += [
            ...
            + (
                "nullptr"
                if not ext["next"]
                else ("(const mesh_gdnative_api_struct *)&" + get_extension_struct_instance_name(name, ext["next"]))
            )
            + ",",
        ]
    ...
    def get_core_struct_definition(core):
        ...
        ret_val += [
            "extern const mesh_gdnative_core_"
            ...
            + (
                ...
                if not core["next"]
                    ...
                    "(const mesh_gdnative_api_struct *)& api_{0}_{1}".format(
    ...
    out += ["", "const mesh_gdnative_api_struct *gdnative_extensions_pointers[] = {"]

    for ext in api["extensions"]:
        name = ext["name"]
        out += ["\t(mesh_gdnative_api_struct *)&api_extension_" + name + "_struct,"]
