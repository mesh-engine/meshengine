    ...
    internal static class DelegateUtils
    {
        ...
        private static bool TrySerializeSingleDelegate(Delegate @delegate, out byte[] buffer)
        {
            ...
            switch (target)
            {
                ...
                case Mesh.Object meshObject:
                {
                    using (var stream = new MemoryStream())
                    using (var writer = new BinaryWriter(stream))
                    {
                        writer.Write((ulong)TargetKind.MeshObject);
                        writer.Write((ulong)meshObject.GetInstanceId());
        ...
        private static bool TryDeserializeSingleDelegate(byte[] buffer, out Delegate @delegate)
        {
            ...
            using (var reader = new BinaryReader(stream))
            {
                ...
                switch (targetKind)
                {
                    ...
                    case TargetKind.MeshObject:
                    {
                        ulong objectId = reader.ReadUInt64();
                        Mesh.Object meshObject = GD.InstanceFromId(objectId);
                        if (meshObject == null)
                            return false;
                        ...
                        @delegate = Delegate.CreateDelegate(delegateType, meshObject, methodInfo);
                        return true;
                    }
