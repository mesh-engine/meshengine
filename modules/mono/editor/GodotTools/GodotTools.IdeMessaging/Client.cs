...
namespace MeshTools.IdeMessaging
{
    ...
    public sealed class Client : IDisposable
    {
        ...
        private MeshIdeMetadata meshIdeMetadata;
        private readonly FileSystemWatcher fsWatcher;

        public string MeshEditorExecutablePath => meshIdeMetadata.EditorExecutablePath;
        ...
        public Client(string identity, string meshProjectDir, IMessageHandler messageHandler, ILogger logger)
        {
            this.identity = identity;
            this.messageHandler = messageHandler;
            this.logger = logger;

            string projectMetadataDir = Path.Combine(meshProjectDir, ".mesh", "mono", "metadata");
            // FileSystemWatcher requires an existing directory
            if (!Directory.Exists(projectMetadataDir)) {
                // Check if the non hidden version exists
                string nonHiddenProjectMetadataDir = Path.Combine(meshProjectDir, "mesh", "mono", "metadata");
            ...
            MetaFilePath = Path.Combine(projectMetadataDir, MeshIdeMetadata.DefaultFileName);

            fsWatcher = new FileSystemWatcher(projectMetadataDir, MeshIdeMetadata.DefaultFileName);
        }
        ...
        private async void OnMetaFileChanged(object sender, FileSystemEventArgs e)
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (metadata != null && metadata != meshIdeMetadata)
                {
                    meshIdeMetadata = metadata.Value;
        ...
        private MeshIdeMetadata? ReadMetadataFile()
        {
            ...
            using (var reader = new StreamReader(fileStream))
            {
                ...
                return new MeshIdeMetadata(port, editorExecutablePath);
            }
        }
        ...
        private async Task ConnectToServer()
        {
            ...
            try
            {
                logger.LogInfo("Connecting to Mesh Ide Server");

                await tcpClient.ConnectAsync(IPAddress.Loopback, meshIdeMetadata.Port);

                logger.LogInfo("Connection open with Mesh Ide Server");
            ...
            catch (SocketException e)
            {
                if (e.SocketErrorCode == SocketError.ConnectionRefused)
                    logger.LogError("The connection to the Mesh Ide Server was refused");
                else
                    throw;
            }
        }
        ...
        public async void Start()
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (!File.Exists(MetaFilePath))
                {
                    logger.LogInfo("There is no Mesh Ide Server running");
                    return;
                }
                ...
                {
                    logger.LogError("Failed to read Mesh Ide metadata file");
                }
            }
        }
            ...
            where TResponse : Response, new()
        {
            if (!IsConnected)
            {
                logger.LogError("Cannot write request. Not connected to the Mesh Ide Server.");
                return null;
            }
