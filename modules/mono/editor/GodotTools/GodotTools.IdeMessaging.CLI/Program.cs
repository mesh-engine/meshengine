...
namespace MeshTools.IdeMessaging.CLI
{
    internal static class Program
    {
        ...
        private static async Task<int> StartAsync(string[] args, Stream inputStream, Stream outputStream)
        {
            ...
            try
            {
                ...
                string meshProjectDir = args[0];

                if (!Directory.Exists(meshProjectDir))
                {
                    Logger.LogError($"The specified Mesh project directory does not exist: {meshProjectDir}");
                    return 1;
                }

                var forwarder = new ForwarderMessageHandler(outputWriter);

                using (var fwdClient = new Client("VisualStudioCode", meshProjectDir, forwarder, Logger))
                {
                    fwdClient.Start();
