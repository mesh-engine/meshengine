using Mesh;
using MeshTools.Core;
using MeshTools.Export;
using MeshTools.Utils;
using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using MeshTools.Build;
using MeshTools.Ides;
using MeshTools.Ides.Rider;
using MeshTools.Internals;
using MeshTools.ProjectEditor;
using JetBrains.Annotations;
using static MeshTools.Internals.Globals;
using File = MeshTools.Utils.File;
using OS = MeshTools.Utils.OS;
using Path = System.IO.Path;

namespace MeshTools
{
    public class MeshSharpEditor : EditorPlugin, ISerializationListener
    {
        ...
        public MeshIdeManager MeshIdeManager { get; private set; }
        ...
        private bool CreateProjectSolution()
        {
            using (var pr = new EditorProgress("create_csharp_solution", "Generating solution...".TTR(), 3))
            {
                ...
                if (guid.Length > 0)
                {
                    ...
                    pr.Step("Updating Mesh API assemblies...".TTR());
                    ...
                    {
                        ShowErrorDialog("Failed to update the Mesh API assemblies: " + debugApiAssembliesError);
                        return false;
                    }
                    ...
                    {
                        ShowErrorDialog("Failed to update the Mesh API assemblies: " + releaseApiAssembliesError);
                        return false;
                    }
        ...
        private void _MenuOptionPressed(int id)
        {
            switch ((MenuOptions)id)
            {
                case MenuOptions.CreateSln:
                    ...
                    break;
                case MenuOptions.SetupMeshNugetFallbackFolder:
                {
                    try
                    {
                        string fallbackFolder = NuGetUtils.MeshFallbackFolderPath;
                        NuGetUtils.AddFallbackFolderToUserNuGetConfigs(NuGetUtils.MeshFallbackFolderName, fallbackFolder);
                        NuGetUtils.AddBundledPackagesToFallbackFolder(fallbackFolder);
                    }
                    catch (Exception e)
                    {
                        ShowErrorDialog("Failed to setup Mesh NuGet Offline Packages: " + e.Message);
                    }
        ...
        {
            if (!File.Exists(MeshSharpDirs.ProjectSlnPath))
            {
        ...
        public Error OpenInExternalEditor(Script script, int line, int col)
        {
            ...
            switch (editorId)
            {
                ...
                case ExternalEditorId.VisualStudio:
                {
                    ...
                    {
                        MeshSharpDirs.ProjectSlnPath,
                        line >= 0 ? $"{scriptPath};{line + 1};{col + 1}" : scriptPath
                    };

                    string command = Path.Combine(MeshSharpDirs.DataEditorToolsDir, "MeshTools.OpenVisualStudio.exe");

                    try
                    {
                        if (Mesh.OS.IsStdoutVerbose())
                ...
                {
                    string scriptPath = ProjectSettings.GlobalizePath(script.ResourcePath);
                    RiderPathManager.OpenFile(MeshSharpDirs.ProjectSlnPath, scriptPath, line);
                    return Error.Ok;
                }
                case ExternalEditorId.MonoDevelop:
                {
                    ...
                    MeshIdeManager.LaunchIdeAsync().ContinueWith(launchTask =>
                    {
        ...
        private void ApplyNecessaryChangesToSolution()
        {
            try
            {
                ...
                DotNetSolution.MigrateFromOldConfigNames(MeshSharpDirs.ProjectSlnPath);

                var msbuildProject = ProjectUtils.Open(MeshSharpDirs.ProjectCsProjPath)
                ...
                ProjectUtils.EnsureMeshSdkIsUpToDate(msbuildProject);

                if (msbuildProject.HasUnsavedChanges)
                {
                    // Save a copy of the project before replacing it
                    FileUtils.SaveBackupCopy(MeshSharpDirs.ProjectCsProjPath);
        ...
        public override void _EnablePlugin()
        {
            ...
            if (File.Exists(MeshSharpDirs.ProjectSlnPath) && File.Exists(MeshSharpDirs.ProjectCsProjPath))
            {
            ...
            _editorSettings.AddPropertyInfo(new Mesh.Collections.Dictionary
            {
            ...
            try
            {
                // At startup we make sure NuGet.Config files have our Mesh NuGet fallback folder included
                NuGetUtils.AddFallbackFolderToUserNuGetConfigs(NuGetUtils.MeshFallbackFolderName, NuGetUtils.MeshFallbackFolderPath);
            }
            catch (Exception e)
            {
                GD.PushError("Failed to add Mesh NuGet Offline Packages to NuGet.Config: " + e.Message);
            }

            BuildManager.Initialize();
            RiderPathManager.Initialize();

            MeshIdeManager = new MeshIdeManager();
            AddChild(MeshIdeManager);
        }
        ...
        public static MeshSharpEditor Instance { get; private set; }

        [UsedImplicitly]
        private MeshSharpEditor()
        {
        }
    }
}
