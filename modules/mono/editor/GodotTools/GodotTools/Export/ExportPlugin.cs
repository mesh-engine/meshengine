...
using Directory = MeshTools.Utils.Directory;
using File = MeshTools.Utils.File;
using OS = MeshTools.Utils.OS;
using Path = System.IO.Path;

namespace MeshTools.Export
{
    public class ExportPlugin : EditorExportPlugin
    {
        ...
        private void AddI18NAssemblies(Mesh.Collections.Dictionary<string, string> assemblies, string bclDir)
        {
        ...
        public void RegisterExportSettings()
        {
            ...
            ProjectSettings.AddPropertyInfo(new Mesh.Collections.Dictionary
            {
        ...
        public override void _ExportFile(string path, string type, string[] features)
        {
            ...
            if (!includeScriptsContent)
            {
                ...
                // Sadly, Mesh prints errors when adding an empty file (nothing goes wrong, it's just noise).
                // Because of this, we add a file which contains a line break.
                AddFile(path, System.Text.Encoding.UTF8.GetBytes("\n"), remap: false);

                // Tell the Mesh exporter that we already took care of the file
                Skip();
            }
        }
        ...
        private void _ExportBeginImpl(string[] features, bool isDebug, string path, int flags)
        {
            ...
            var assemblies = new Mesh.Collections.Dictionary<string, string>();

            string projectDllName = MeshSharpEditor.ProjectAssemblyName;
            string projectDllSrcDir = Path.Combine(MeshSharpDirs.ResTempAssembliesBaseDir, buildConfig);
            ...
            {
                string meshAndroidExtProfileDir = GetBclProfileDir("mesh_android_ext");
                string monoAndroidAssemblyPath = Path.Combine(meshAndroidExtProfileDir, "Mono.Android.dll");
            ...
            string resAssembliesDir = Path.Combine(MeshSharpDirs.ResAssembliesBaseDir, apiConfig);
        ...
        public override void _ExportEnd()
        {
            ...
            string aotTempDir = Path.Combine(Path.GetTempPath(), $"mesh-aot-{Process.GetCurrentProcess().Id}");
            ...
            if (!string.IsNullOrEmpty(_maybeLastExportError)) // Check empty as well, because it's set to empty after hot-reloading
            {
                ...
                MeshSharpEditor.Instance.ShowErrorDialog(lastExportError, "Failed to export C# project");
            }
        }
        ...
        {
            // OSX export templates are contained in a zip, so we place our custom template inside it and let Mesh do the rest.
        ...
        /// <summary>
        /// Determines whether the BCL bundled with the Mesh editor can be used for the target platform,
        ...
        private static extern void internal_GetExportedAssemblyDependencies(Mesh.Collections.Dictionary<string, string> initialAssemblies,
            string buildConfig, string customBclDir, Mesh.Collections.Dictionary<string, string> dependencyAssemblies);
    }
}
