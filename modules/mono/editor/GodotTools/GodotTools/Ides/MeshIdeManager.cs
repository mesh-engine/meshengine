...
{
    public sealed class MeshIdeManager : Node, ISerializationListener
    {
        ...
        private MessagingServer GetRunningOrNewServer()
        {
            ...
            _messagingServer = new MessagingServer(OS.GetExecutablePath(), ProjectSettings.GlobalizePath(MeshSharpDirs.ResMetadataDir), new MeshLogger());
        ...
        {
            var editorId = (ExternalEditorId)MeshSharpEditor.Instance.GetEditorInterface()
        ...
        private void LaunchIde(ExternalEditorId editorId, string editorIdentity)
        {
            switch (editorId)
            {
                ...
                case ExternalEditorId.MonoDevelop:
                {
                    ...
                    try
                    {
                        var instance = GetMonoDevelopInstance(MeshSharpDirs.ProjectSlnPath);
        ...
        public readonly struct EditorPick
        {
            ...
            public bool IsAnyConnected() =>
                MeshSharpEditor.Instance.MeshIdeManager.GetRunningOrNewServer().IsAnyConnected(_identity);
                ...
                where TResponse : Response, new()
            {
                ...
                MeshSharpEditor.Instance.MeshIdeManager
        ...
        private class MeshLogger : ILogger
        {
