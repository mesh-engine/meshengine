...
- All cpp files listed in `modules/raycast/mesh_update_embree.py`
- All header files in the directories listed in `modules/raycast/mesh_update_embree.py`

The `modules/raycast/mesh_update_embree.py` script can be used to pull the
relevant files from the latest Embree release and apply some automatic changes.

Some changes have been made in order to remove exceptions and fix minor build errors.
They are marked with `// -- MESH start --` and `// -- MESH end --`
...
to be usable by Mesh's socket implementation and allow IPv6 and DTLS.
Apply the patches in the `patches/` folder when syncing on newer upstream
commits.

Three files (mesh.cpp, enet/mesh.h, enet/mesh_ext.h) have been added to provide
enet socket implementation using Mesh classes.
...
- the `icudt70l.dat` built with the provided `mesh_data.json` config file (see
  https://github.com/unicode-org/icu/blob/master/docs/userguide/icu_data/buildtool.md
  for instructions).

- Step 1: Build ICU with default options - `./runConfigureICU {PLATFORM} && make`.
- Step 2: Reconfigure ICU with custom data config - `ICU_DATA_FILTER_FILE={MESH_SOURCE}/thirdparty/icu4c/mesh_data.json ./runConfigureICU {PLATFORM} --with-data-packaging=common`.
- Step 3: Delete `data/out` folder and rebuild data - `cd data && rm -rf ./out && make`.
- Step 4: Copy `source/data/out/icudt70l.dat` to the `{MESH_SOURCE}/thirdparty/icu4c/icudt70l.dat`.
...
Important: The files `utils/bit_reader_utils.{c,h}` have Mesh-made
changes to ensure they build for Javascript/HTML5. Those
changes are marked with `// -- MESH --` comments.
...
- Added 2 files `mesh_core_mbedtls_platform.c` and `mesh_core_mbedtls_config.h`
...
The only modified file is `src/miniupnpcstrings.h`, which was created for Mesh
...
Important: Some files have Mesh-made changes for use in core/io.
They are marked with `/* MESH start */` and `/* MESH end */`
...
Collection of single-file libraries used in Mesh components.

- `clipper.{cpp,hpp}`
  * Upstream: https://sourceforge.net/projects/polyclipping
  * Version: 6.4.2 (2017) + Mesh changes (added optional exceptions handling)
  * License: BSL-1.0
- `cubemap_coeffs.h`
...
- `polypartition.{cpp,h}`
  ...
  * Modifications: Change from STL to Mesh types (see provided patch).
  * License: MIT
- `r128.h`
...
Modified files:
Modifications are marked with `// -- MESH start --` and `// -- MESH end --`.
...
Important: Some files have Mesh-made changes; so to enrich the features
originally proposed by this library and better integrate this library with
Mesh. Please check the file to know what's new.
...
`// -- MESH begin --` and `// -- MESH end --` comments.
...
Important: Some files have Mesh-made changes.
They are marked with `// -- MESH start --` and `// -- MESH end --`
...
Some downstream changes have been made and are identified by
`// -- MESH start --` and `// -- MESH end --` comments.
...
`vk_mem_alloc.cpp` is a Mesh file and should be preserved on updates.
...
- Run `cmake .` to generate `config.h` and `wslayver.h`.
  Contents might need tweaking for Mesh, review diff.
- All `*.c` and `*.h` files from `lib/`
- All `*.h` in `lib/includes/wslay/` as `wslay/`
- `wslay/wslay.h` has a small Mesh addition to fix MSVC build.
