...
def get_build_version():
    ...
    if os.getenv("MESH_VERSION_STATUS") != None:
        status = str(os.getenv("MESH_VERSION_STATUS"))
    v += ".%s.%s" % (status, name)
    return v
...
def create_template_zip(env, js, wasm, extra):
    binary_name = "mesh.tools" if env["tools"] else "mesh"
    ...
    if env["tools"]:
        ...
        cache = [
            ...
            "offline.html",
            "mesh.tools.js",
            "mesh.tools.worker.js",
            "mesh.tools.audio.worklet.js",
            "logo.svg",
            "favicon.png",
        ]
        opt_cache = ["mesh.tools.wasm"]
        subst_dict = {
            "@MESH_VERSION@": get_build_version(),
            "@MESH_NAME@": "GodotEngine",
            "@MESH_CACHE@": json.dumps(cache),
            "@MESH_OPT_CACHE@": json.dumps(opt_cache),
            "@MESH_OFFLINE_PAGE@": "offline.html",
        }
        html = env.Substfile(target="#bin/mesh${PROGSUFFIX}.html", source=html, SUBST_DICT=subst_dict)
        ...
        service_worker = env.Substfile(
            target="#bin/mesh${PROGSUFFIX}.service.worker.js", source=service_worker, SUBST_DICT=subst_dict
        )
        ...
        out_files.append(zip_dir.File("mesh.offline.html"))
