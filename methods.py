...
def update_version(module_version_string=""):
    ...
    mesh_status = str(version.status)
    if os.getenv("MESH_VERSION_STATUS") != None:
        mesh_status = str(os.getenv("MESH_VERSION_STATUS"))
        print("Using version status '{}', overriding the original '{}'.".format(mesh_status, str(version.status)))
    f.write('#define VERSION_STATUS "' + mesh_status + '"\n')
    f.write('#define VERSION_BUILD "' + str(build_name) + '"\n')
    f.write('#define VERSION_MODULE_CONFIG "' + str(version.module_config) + module_version_string + '"\n')
    f.write("#define VERSION_YEAR " + str(version.year) + "\n")
    f.write('#define VERSION_WEBSITE "' + str(version.website) + '"\n')
    f.write('#define VERSION_DOCS_BRANCH "' + str(version.docs) + '"\n')
    f.write('#define VERSION_DOCS_URL "https://mesh-engine.com/en/" VERSION_DOCS_BRANCH\n')
...
def detect_modules(search_path, recursive=False):
    ...
    def is_engine(path):
        ...
        # Mesh sources when using `custom_modules` build option.
        version_path = os.path.join(path, "version.py")
        if os.path.exists(version_path):
            with open(version_path) as f:
                if 'short_name = "mesh"' in f.read():
                    return True
        return False
...
def detect_visual_c_compiler_version(tools_env):
    ...
    # There are other architectures, but Mesh does not support them currently, so this function does not detect arm/amd64_arm
...
def generate_vs_project(env, num_jobs):
    ...
    if batch_file:
        ...
        class ModuleConfigs(Mapping):
            ...
            def add_mode(
                ...
                self.arg_dict["runfile"] += [
                    f'bin\\mesh.windows.{config_id}.{plat_id}{f".{name}" if name else ""}.exe'
        ...
        env.MSVSProject(
            target=["#mesh" + env["MSVSPROJECTSUFFIX"]],
            incs=env.vs_incs,
            srcs=env.vs_srcs,
